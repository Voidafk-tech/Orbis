
import React from 'react';
import { Page } from '../App';

interface GrowthPageProps {
  navigate: (page: Page) => void;
}

const GrowthPage: React.FC<GrowthPageProps> = ({ navigate }) => {
  return (
    <div className="py-20 animate-in fade-in duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-gray-900 dark:bg-gray-800 rounded-3xl p-12 lg:p-20 text-white overflow-hidden relative mb-20">
          <div className="absolute top-0 right-0 w-96 h-96 bg-primary/20 rounded-full blur-[120px]"></div>
          <div className="relative z-10 max-w-3xl">
            <h1 className="text-5xl lg:text-7xl font-display font-extrabold mb-8 leading-tight">Scale Without the <span className="text-primary">Overhead.</span></h1>
            <p className="text-xl text-gray-300 leading-relaxed mb-10">
              The biggest challenge for scaling B2B companies is building a finance team that can keep up. 
              Orbis Accounting provides fractional senior accounting services that scale with you—so you only pay for the expertise you need.
            </p>
            <button onClick={() => navigate('contact')} className="px-10 py-5 bg-primary text-gray-900 font-extrabold rounded-2xl hover:scale-105 transition-transform text-lg">
              Unlock Your Growth Strategy
            </button>
          </div>
        </div>

        <div className="grid md:grid-cols-2 gap-16">
          <div className="space-y-6">
            <h2 className="text-3xl font-bold">Scaling Metrics That Matter</h2>
            <p className="text-gray-400">We don't just track profit; we track efficiency. Our growth reporting includes:</p>
            <ul className="space-y-4">
              {[
                "Cash Runway Forecasting",
                "Customer Acquisition Cost (CAC) Analysis",
                "Operating Margin Optimization",
                "Tax-efficient Scaling Strategies"
              ].map((item, i) => (
                <li key={i} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">
                  <span className="material-icons-outlined text-primary">trending_up</span>
                  <span className="font-bold">{item}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="bg-primary/5 p-10 rounded-3xl border border-primary/20 flex flex-col justify-center">
            <h3 className="text-2xl font-bold mb-4">Start Your Growth Journey</h3>
            <p className="text-gray-400 mb-8 leading-relaxed">
              Don't let manual bookkeeping slow down your acquisition or expansion plans. 
              Get a financial partner that understands the velocity of a growing business.
            </p>
            <button onClick={() => navigate('contact')} className="w-full py-4 bg-gray-900 dark:bg-white dark:text-gray-900 text-white font-bold rounded-xl transition-colors">
              Request Growth Audit
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default GrowthPage;
